...
function updateUserCredit($userId, $amount) {
    global $conn;
    $sql = "UPDATE money SET mon_price = mon_price + ? WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("di", $amount, $userId);
    $result = $stmt->execute();
    $newCredit = 0;
    if ($result) {
        $sql = "SELECT mon_price FROM money WHERE user_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $stmt->bind_result($newCredit);
        $stmt->fetch();
    }
    $stmt->close();
    return ['success' => $result, 'newCredit' => $newCredit];
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'updateCredit') {
    // ตรวจสอบว่า user ได้เข้าสู่ระบบหรือไม่
    if (isLoggedIn()) {
        $userId = $_SESSION['user_id'];  // ดึง userId จาก session โดยตรง
        $amount = $_POST['amount'];  // จำนวนเครดิตที่ต้องการอัพเดต เช่น -10 เพื่อหักเครดิต

        // ดึงเครดิตปัจจุบันของผู้ใช้
        $userCredit = getUserCredit($userId);

        // เช็คว่าเครดิตเพียงพอสำหรับการหักหรือไม่
        if ($userCredit + $amount >= 0) {
            $result = updateUserCredit($userId, $amount);
...
function addCredit($userId, $amount) {
    global $conn;
    $currentCredit = getUserCredit($userId);
    $newCredit = $currentCredit + $amount;
    $sql = "UPDATE money SET mon_price = ? WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'insertIntoCatalog') {
    ...
    if (isLoggedIn()) {
        $userId = $_SESSION['user_id'];  // ดึง userId จาก session
        $productId = $_POST['productId'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'addCredit') {
    $userId = $_POST['userId'];
    $amount = $_POST['amount'];
    $result = addCredit($userId, $amount);
    echo json_encode(['success' => $result]);
    exit;
}
...
<body>
    ...
    <div class="slot-machine">
        ...
        <div class="money-pay">
            <?php if (isLoggedIn()): ?>
                เครดิตของคุณ: <span id="userCredit"><?php echo $userCredit; ?></span>
            <?php else: ?>
                กรุณาเข้าสู่ระบบเพื่อดูเครดิตของคุณ
            <?php endif; ?>
        </div>
        <div class="laem01">สุ่มรางวัลเริ่มต้นเพียง 10 เครดิตเท่านั้น</div>
    ...
    <script>
        ...
        let userCredit = <?php echo $userCredit; ?>;
        ...
        function spinReels() {
            ...
            if (!isLoggedIn) {
                Swal.fire({
                    ...
                    text: 'คุณต้องเข้าสู่ระบบก่อนจึงจะสามารถสุ่มรางวัลได้',
                    icon: 'warning',
            ...
            if (userCredit < 10) {
                Swal.fire({
                    ...
                    text: 'คุณต้องมีอย่างน้อย 10 เครดิตเพื่อเล่น',
                    icon: 'error',
                    confirmButtonText: 'ตกลง'
                });
                return;
    ...
    // Remove credit deduction from here
...
function updateCreditInDatabase(amount) {
    fetch('<?php echo $_SERVER['PHP_SELF']; ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `action=updateCredit&amount=${amount}` // ไม่ต้องส่ง userId
    })
    .then(response => response.json())
    .then(data => {
        if (!data.success) {
            console.error('Failed to update credit in database');
        }
    });
}
...
function stopReel(reelIndex) {
    ...
    if (results[0].id === results[1].id && results[1].id === results[2].id) {
        Swal.fire({
            ...
            deductCreditAndInsertCatalog(results[0].id);
        });
    } else {
        Swal.fire({
            title: 'เสียใจด้วย',
            ...
            deductCreditAndInsertCatalog();
        });
    }
}
function deductCreditAndInsertCatalog(productId = null) {
    userCredit -= 10;
    document.getElementById('userCredit').textContent = userCredit;

    // Update credit in the database
    updateCreditInDatabase(-10);

    if (productId) {
        const userId = <?php echo $_SESSION['user_id']; ?>; // ดึง userId จาก session
